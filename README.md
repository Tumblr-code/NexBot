...
| 命令 | 描述 | 权限 | 别名 |
|------|------|------|------|
| `help` | 显示帮助信息 | 所有人 | `h`, `start` |
| `help <命令>` | 查看指定命令详情 | 所有人 | - |
| `ping` | 测试响应速度 | 所有人 | `pong` |
| `echo` | 回声测试 | 所有人 | `say` |
| `id` | 获取当前聊天信息 | 所有人 | `chatid`, `chat` |
| `sysinfo` | 系统信息（内存、CPU、运行时间） | 所有人 | `status`, `stats`, `info` |
| `uptime` | 显示运行时间 | 所有人 | `up` |
| `health` | 健康状态检查 | 所有人 | `hc` |
| `exec` | 执行 Shell 命令 | sudo | `shell`, `sh`, `cmd` |
| `eval` | 执行 JavaScript 代码 | sudo | `js` |
| `db` | 数据库信息 | sudo | `database` |
| `cache` | 缓存统计 | sudo | - |
| `ratelimit` | 限流统计 | sudo | `rl` |
| `msg` | 获取消息原始数据（调试） | sudo | - |
| `plugin` | 插件管理 | sudo | `pm`, `plugins` |
...
| 插件 | 命令 | 描述 | 权限 |
|------|------|------|------|
| **ai** | `ai` | AI 对话助手（支持 OpenAI/Gemini/Claude） | 所有人 |
| **pansou** | `pan` | 网盘搜索（Telegraph 排版） | 所有人 |
| **privateguard** | `pglist` | 私聊保护（人机验证） | sudo |
| **speedtest** | `speedtest` | 网速测试 | sudo |
